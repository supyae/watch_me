                            ...
                            <div class="form-group">
                                ...
                                @if($col == $relationKey)
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        {!! Form::select($col,array_pluck($dropdown, 'name', 'id'), $data->$col, ['class' => 'form-control']) !!}
                                    </div>
                                @elseif(strpos($col, '_img') !== false )
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        {!! Form::file('upload',['class'=>'content-box-large']) !!}
                                        <img src="{{$data->$col}}" width="250px" height="200px">
                                    </div>
                                @elseif(strpos($col, '_date') !== false)
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        <input id="{{ $col }}" name="{{ $col }}"
                                               class="date-picker form-control col-md-7 col-xs-12" placeholder="m/d/Y"
                                               required="required" value="{{ date('m/d/Y',strtotime($data->$col)) }}"
                                               type="text">
                                    </div>
                                @else
                                    <div class="col-md-6 col-sm-6 col-xs-12">
